...
(function clean() {
    ...
    waitForKeyElements('div[class="DraftEditor-editorContainer"]', function() {
        cleanDescription();
        var observer = new MutationObserver(cleanDescription);
        observer.observe(document.querySelector('div[class="DraftEditor-editorContainer"]'), {attributes: false, childList: true, characterData: false, subtree: false});
    });
    ...
    waitForKeyElements('div[class="footerButtons"]', function() {
        cleanFooter();
    });

    waitForKeyElements('div[data-test-id="board-header"]', function() {
